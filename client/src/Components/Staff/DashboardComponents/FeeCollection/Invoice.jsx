...
const Invoice = () => {
  ...
  const getstdentData = async () => {
    try {
      const response = await axiosPrivate.put(`/admission/${id}`);
      console.log(response.data);

      setAdmssionstdntData(response.data);
    } catch (error) {
      console.log(error);
    }
  };
  const getInvoiceDetails = async () => {
    try {
      const response = await axiosPrivate.put("/accounts/filter", {
        admnId: id,
      });

      setInvoiceData(response.data[0].admissionFees);
      console.log(response.data[0].admissionFees);
    } catch (error) {
      console.log(error);
    }
  };

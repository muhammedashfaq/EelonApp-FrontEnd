...
const AddBusRoutes = () => {
  ...
  return (
    <>
      <div className='px-5'>
        <div className='h-20 bg-gray-600 w-full shadow-2xl mt-10 flex justify-between items-center px-5' style={{borderRadius: '15px'}}>
          ...
          <div className='flex flex-col'>
            ...
            <Typography variant='small'>Registration no:{selectedBus?.rgNo}</Typography>
          </div>
                      ...
                      <Draggable key={item?.id} draggableId={item?.id} index={index}>
                        {provided => (
                          <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps}>
                            <BusRouteRow key={item?.id} item={item} handleData={handleData} removeDiv={removeDiv} index={index + 1} />
                          </div>
                        )}
                      </Draggable>
                    ))}

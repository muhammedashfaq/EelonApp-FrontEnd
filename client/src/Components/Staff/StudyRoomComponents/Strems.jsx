...
const Strems = ({classRoomData}) => {
  ...
  const deletAnnouncement =async(id)=>{
    try {
      ...
      console.log(id,"id");
      console.log(classroomId,'clss');
      const response = await axiosPrivate.delete(`/classroom/announcement/${classroomId}`,{ data: { deleteId: id }});
      if(response){
        getAnnouncements()
...
try {
  const response = await axiosPrivate.get(`/classroom/announcement/${classroomId}`);
            ...
            announcement?.map((item,i)=>(
              ...
              <PopoverHandler>
                <Card className=" bg-white shadow-md rounded-lg overflow-hidden hover:shadow-xl ">
                  <X className="cursor-pointer hover:bg-blue-gray-100 rounded-md bg-red-300" onClick={()=>deletAnnouncement(item._id)}/>
                  <CardBody className="">
                    <div className="justify-between items-center bg-gray-200">
                        <div className="flex justify-center items-center">{item?.topic}</div>

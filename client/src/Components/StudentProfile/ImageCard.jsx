...
import {faEdit, faEnvelope, faEye, faLock, faLockOpen, faTrash, faUpload} from '@fortawesome/free-solid-svg-icons';
...
import UploadStudentImageModal from './UploadStudentImageModal';
...
const ImageCard = ({userData, getData}) => {
  ...
  const uploadImage = async () => {
    try {
      ...
      setIsLoading(true);
      await axiosPrivate.post(`images/studentprofile/${userData?._id}`, {Image});
      setIsLoading(false);
  ...
  return (
    <div>
      <Card className='w-96 h-full max-h-[30rem] cursor-pointer '>
        <CardHeader shadow={false} floated={false} className='h-96' onClick={handleOpen}>
          {isLoading ? (
            <>
            ...
            <img src={userData?.profilePic?.url || Image || notFoundImg} alt='card-image' className='h-full w-full object-contain' />
          )}
        </CardHeader>
        ...
        {(auth.roles == 5151 || auth.roles == 2000) && (
          <div className='space-x-2 flex justify-center my-1 items-center'>
            {/* <Button color='teal'>
              ...
              <input type='file' id='imageUpload' name='profileimage' className='hidden' onChange={handleImage} />
            </Button> */}
            <UploadStudentImageModal getData={getData} userId={userData?._id} />

            {/* <Button color='lime' className='' onClick={uploadImage}>
              <FontAwesomeIcon icon={faUpload} size='2xl' />
            </Button> */}
            <Button color='red' className='' onClick={deleteImage} disabled={userData?.studentProfilePic ? false : true}>
      ...
      <Dialog size='lg' open={open} handler={handleOpen}>
        ...
        <DialogFooter className='justify-between'>
          ...
          <Button size='sm' variant='outlined' color='blue-gray' className='mr-5 flex items-center'>
            Share
          </Button>
        </DialogFooter>
      </Dialog>
